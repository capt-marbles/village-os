...
test("allows only official HTTPS LinkedIn navigation in the isolated view", () => {
  assert.deepEqual(decideNavigation("https://www.linkedin.com/login"), {
    action: "allow",
  });
...
test("authentication verification is conservative and preserves owner confirmation", () => {
  assert.deepEqual(
    verifyAuthentication("https://www.linkedin.com/login", false),
    {
  ...
  assert.deepEqual(
    verifyAuthentication("https://www.linkedin.com/feed/", false),
    {
      status: "unknown",
      predicateVersion: "linkedin-url-v1",
    },
  ...
  assert.deepEqual(
    verifyAuthentication("https://www.linkedin.com/feed/", true),
    {
  ...
  assert.equal(
    verifyAuthentication("https://evil.example/feed/", true).status,
    "unknown",
  );
});

test("allows debugger attachment only to an owned loopback fixture", () => {
  assert.deepEqual(decideDebuggerTarget("http://127.0.0.1:4173/auth"), {
    action: "allow",
  });
  assert.deepEqual(decideDebuggerTarget("http://localhost:4173/auth"), {
    action: "allow",
  });
...
test("classifies human auth routes explicitly and visibly", () => {
  assert.equal(
    classifyAuthRoute("https://www.linkedin.com/checkpoint/challenge"),
    "human-challenge",
  );
  assert.equal(
    classifyAuthRoute("https://www.linkedin.com/checkpoint/lg/login-submit"),
    "human-2fa",
  );
  assert.equal(
    classifyAuthRoute("https://www.linkedin.com/uas/request-password-reset"),
    "human-password-reset",
  );
  assert.equal(
    classifyAuthRoute("https://accounts.google.com/o/oauth2/auth"),
    "unsupported-federated",
  );
  assert.equal(
    classifyAuthRoute("https://www.linkedin.com/passkey"),
    "unsupported-passkey",
  );
  assert.equal(
    classifyAuthRoute("https://www.linkedin.com/legal/user-agreement"),
...
test("denies every remote permission and popup without runtime exceptions", () => {
  assert.deepEqual(decidePermission("media"), {
    action: "deny",
...
test("production-intended remote configuration is isolated and hardened", () => {
  assert.equal(
    LINKEDIN_PARTITION,
    "persist:village-principal-local-device-local-linkedin",
  );

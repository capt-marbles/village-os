...
test("profile posture fails closed when OS encryption support is unavailable", () => {
  assert.deepEqual(
    evaluateProfilePosture({ encryptionAvailable: false, platform: "darwin" }),
    {
      ok: false,
      warning:
...
test("profile posture rejects unsupported alpha platforms", () => {
  assert.equal(
    evaluateProfilePosture({ encryptionAvailable: true, platform: "linux" }).ok,
    false,
  );
...
test("profile posture accepts supported macOS with OS encryption", () => {
  assert.deepEqual(
    evaluateProfilePosture({ encryptionAvailable: true, platform: "darwin" }),
    { ok: true },
  );
});

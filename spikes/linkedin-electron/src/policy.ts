export const LINKEDIN_PARTITION =
...
export type AuthenticationStatus =
  "authenticated" | "confirmed_by_user" | "not_authenticated" | "unknown";
...
export function classifyAuthRoute(raw: string): AuthRoute {
  ...
  if (route.includes("passkey") || route.includes("webauthn"))
    return "unsupported-passkey";
  if (
    route.includes("login-submit") ||
    route.includes("two-step") ||
    route.includes("two_factor")
  )
    return "human-2fa";
  if (
    route.includes("checkpoint") ||
    route.includes("challenge") ||
    route.includes("captcha")
  )
    return "human-challenge";
  if (route.includes("password-reset") || route.includes("request-password"))
    return "human-password-reset";
  if (
    route.includes("legal") ||
    route.includes("consent") ||
    route.includes("terms")
  )
    return "human-terms-or-consent";
  return "standard";
}
...
export function verifyAuthentication(
  ...
  if (!url || url.protocol !== "https:" || !linkedinHosts.has(url.hostname)) {
    return {
      status: "unknown",
      predicateVersion: authenticationPredicateVersion,
    };
  }
  if (
  ...
  if (
    ownerConfirmed &&
    (url.pathname.startsWith("/feed") || url.pathname.startsWith("/in/"))
  ) {
    return {
...
export function decideNavigation(raw: string): PolicyDecision {
  ...
  if (url.protocol !== "https:")
    return { action: "deny", reason: "https-required" };
  if (!linkedinHosts.has(url.hostname))
    return { action: "deny", reason: "origin-not-allowlisted" };
  return { action: "allow" };
}
